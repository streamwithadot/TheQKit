...
//    var animationView : LOTAnimationView?
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if(type == .Question){
            let animationView = AnimationView(name: "timer", bundle: TheQKit.bundle)
...
//                #endif
            }else if (self.result?.choices?[1].id == self.result?.selection) {
                // set progress view to red
//                #if !NEWSCORPUK
...
//                #endif
            ...
            if(self.result != nil){
                ...
                }else if (self.result?.choices?[1].correct)!{
                    // set progress view collor to green
