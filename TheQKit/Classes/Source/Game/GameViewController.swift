    ...
    func setUpEventSource() {
        ...
        eSource?.addEventListener("GameWon", handler: { (e) in
            ...
            DispatchQueue.main.async(execute: {
                ...
                let message = "You WON \(asd ?? " ")! Congrats and thanks for playing \(TQKConstants.appName)! Your balance usually updates within 5 minutes."
...
//        print("My RTMP Is " + self.rtmpUrl)
        var urlString:String
        if(TQKConstants.DAN_BRAND_MODE){
            //Harcoded
            urlString = "rtmp://rtmp.stream.live:1935/live/8bc25536-5afd-4410-9d9e-9df7d89c2a9e"
        }else{
            urlString = self.rtmpUrl as String
        }
        
        initializePlayer(url: urlString)
        print(urlString + " my rtmp url to stream is ")
...
//        }
    ...
    func stopStreamAndReset() {
        ...
        //        self.perform(#selector(self.player.play), with: nil, afterDelay: 1.0)
        ...
        //
        self.initializePlayer(url: urlString)
